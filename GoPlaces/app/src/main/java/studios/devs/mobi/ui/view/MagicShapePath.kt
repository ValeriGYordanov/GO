...
import studios.devs.mobi.utils.GeometryUtils
...
class MagicShapePath private constructor(val startX: Float, val startY: Float, val endX: Float, val endY: Float) {
    ...
    private fun getTangentForTwoCircles(circle: CircleShape,
        ...
        if (circle.pathDirection == nextCircle.pathDirection) {
            tangent = if (circle.pathDirection == PathDirection.C_CLOCKWISE) tangents[0] else tangents[1]
        } else {
            tangent = if (circle.pathDirection == PathDirection.C_CLOCKWISE) tangents[2] else tangents[3]
        }
        return tangent
    }
